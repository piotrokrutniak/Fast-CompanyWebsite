...
export default function CarouselPage(){
    ...
    return(
        ...
        <div className={`relative w-full max-w-7xl h-128 flex overflow-visible gap-4 -translate-x-[${activeIndex * 100}%] transition-all`}>
            {slides.map((image, index) => <CarouselPanel image={image.img} key={index} index={index} />)}
        </div>
        <div className='flex w-full justify-between center h-full place-items-center absolute p-2'>
            <div className='bg-black/10 p-2 rounded-2xl block hover:bg-black/20 active:scale-95'> {/*sm:hidden group-hover:*/}
...
function CarouselPanel({image, index}){
    return (
        <div className={`w-full h-full absolute left-[${index * 100}%]`}>
            <Image src={image} layout='fill' objectFit='cover' className="h-full opacity-60"/>
        </div>
        ) 
}