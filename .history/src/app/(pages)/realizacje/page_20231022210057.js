    ...
    return(
        <main className="min-h-screen scroll-smooth">
            <section id="Realizacje" className="max-w-7xl m-auto flex flex-col gap-5">
                <div>
                    <h1 className="text-3xl sm:text-4xl font-semibold lg:p-16 border-b-2 border-slate-600/30">Realizacje</h1>
                </div>
                {data.posts.map((x) => <BlogPost key={x.id} title={x.title} location={x.location} desc={x.description} photos={x.photos}/>)}
            </section>
        </main>
    )
}
