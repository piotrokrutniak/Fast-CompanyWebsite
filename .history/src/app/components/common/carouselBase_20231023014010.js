...
export default function CarouselBase({ slides, alt, className = "", onClick = () => undefined, thumbnail = false }) {
    ...
    return(
        ...
        <div className={`${className} w-full m-auto h-full max-w-7xl`}> 
            <div className='w-full m-auto h-full max-w-7xl items-center flex group transition-all relative' >
                <div className='absolute w-screen justify-center flex !h-full select-none'>
                    <Image src={slides[slideIndex].url} alt={alt || "Placeholder Text"} layout='fill' objectFit='contain' className={`z-10 py-8 ${!thumbnail || "cursor-zoom-in"}`} onClick={() => onClick()}/>
                </div>
                <div className='flex w-full justify-between relative p-2'>
                <div className={`bg-black/10 p-2 rounded-2xl ${thumbnail || "!block"} sm:hidden group-hover:block hover:bg-black/20 active:scale-95 z-20`}>
            ...
            <div className='flex justify-center relative gap-2 -top-10 transition-all z-30'>
