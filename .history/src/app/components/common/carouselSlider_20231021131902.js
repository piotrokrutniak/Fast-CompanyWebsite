...
export function CarouselSlider({slides}){
    ...
    return(
        <>
        <div className="w-full max-w-7xl absolute mx-auto overflow-clip group">
...
function CarouselPanel({image, index}){
    ...
    return (
        <div className={`w-full h-full absolute left-[${index * 100}%] `}>
            <Image src={image} alt={"carousel-img-"+index} layout='fill' objectFit='cover' className="h-full"/>
        </div>
        ) 
}