...
export default function SimplePanel({...props}){
    return(
        <section id={props.id} className="z-10 w-full items-center justify-between text-sm lg:flex p-5 max-xs:p-5">
            ...
            <div className='lg:w-96 h-screen-1/2 lg:h-96 shrink-0 bg-slate-500 rounded-xl relative self-center w-full overflow-clip'>
                <Image src={props.img} alt={props.title} loading="lazy" layout='fill' objectFit='cover'/>
            </div>
            <div className='relative self-center w-full'>
                ...
                <h2 className='text-xl font-thin leading-9 max-sm:text-lg opacity-80'>{props.desc ?? "Description"}</h2>
            </div>
            </div>
        </section>
    )
