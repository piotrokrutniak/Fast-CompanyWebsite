...
export function CarouselSlider({slides}){
    ...
    return(
        ...
        <div className={`relative w-full max-w-7xl h-128 flex overflow-visible -translate-x-[${activeIndex * 100}%] transition-all`}>
            {slides.map((image, index) => <CarouselPanel image={image} key={index} index={index} offset1={index * 100}/>)}
        </div>
...
function CarouselPanel({image, index, offset1}){
    const offset = index * 100
    return (
        <div className={`w-full h-full absolute left-[${offset1}%] `}>
